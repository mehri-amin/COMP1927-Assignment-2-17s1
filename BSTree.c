...
// taken from week 10 lab - https://cgi.cse.unsw.edu.au/~cs1927/17s1/labs/week10/index.php
...
static
BSTLink newBSTNode(char *key, List value)
{
...
BSTree BSTreeInsert(BSTree t, char *key, List val)
{
	if (t == NULL)
...
// get value for key in a BSTree, or NULL
List BSTreeFind(BSTree t, char *key)
{
	if (t == NULL)
		return NULL;
	int res = strcmp(key, t->key);
	if (res < 0)
		return BSTreeFind(t->left, key);
	else if (res > 0)
		return BSTreeFind(t->right, key);
	else // (v == t->value)
		return t->value;
}
...
BSTree BSTreeDelete(BSTree t, char *k)
{
	if (t == NULL)
		return NULL;
	int res = strcmp(k, t->key);
	if (res < 0)
